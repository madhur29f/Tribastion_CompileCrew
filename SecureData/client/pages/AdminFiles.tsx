...
import { Eye, Download, Trash2, Search, Filter, HardDrive, ShieldCheck, AlertTriangle } from "lucide-react";
    ...
    <AdminLayout>
      ...
      >
            ...
            <div className="flex gap-4">
              <div className="bg-background/50 backdrop-blur-md border border-white/10 rounded-xl p-3 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="font-mono font-bold text-foreground">1.2 TB</p>
                </div>
              </div>
              <div className="bg-background/50 backdrop-blur-md border border-white/10 rounded-xl p-3 flex items-center gap-3">
                <ShieldCheck className="w-6 h-6 text-accent" />
                <div>
                  <p className="text-[10px] uppercase font-bold text-muted-foreground">Cleaned</p>
                  <p className="font-mono font-bold text-foreground">{files.filter(f => f.status === 'Completed').length}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <motion.div variants={itemVariants} className="flex flex-col sm:flex-row gap-4">
          ...
          <button className="flex items-center justify-center gap-2 px-6 py-3 bg-secondary hover:bg-secondary/80 border-2 border-border rounded-xl transition-all font-bold text-foreground shadow-sm hover:border-primary/50">
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border bg-background/50">
                  ...
                  <th className="px-6 py-4 text-left text-xs font-bold text-destructive uppercase tracking-wider flex items-center gap-1">
                    <AlertTriangle className="w-3 h-3" /> PII Targets
                  </th>
          ...
          {filteredFiles.length > 0 && (
            <div className="px-6 py-4 bg-secondary/20 flex items-center justify-between border-t border-border">
              ...
              <div className="flex gap-2">
                <button className="px-4 py-2 border border-border rounded-lg hover:bg-white/5 hover:border-primary transition-all text-sm font-bold shadow-sm">
                  &lt; Prev
                </button>
                <button className="px-4 py-2 border border-border rounded-lg hover:bg-white/5 hover:border-primary transition-all text-sm font-bold shadow-sm">
                  Next &gt;
